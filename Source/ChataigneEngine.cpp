...
void ChataigneEngine::importSelection(File f)
{
	...
	{
		/*
		FileChooser fc("加载一个 ICC", File::getCurrentWorkingDirectory(), "*.ICC");
		if (!fc.browseForFileToOpen()) return;
		f = fc.getResult();
*/
		
		FileChooser* fc(new FileChooser(L"加载一个 ICC", File::getCurrentWorkingDirectory(), "*.ICC"));
...
void ChataigneEngine::exportSelection()
{
	...
	FileChooser* fc(new FileChooser(L"保存为 ICC", File::getCurrentWorkingDirectory(), "*.ICC"));
