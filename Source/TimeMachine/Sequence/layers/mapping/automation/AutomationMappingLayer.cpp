...
AutomationMappingLayer::AutomationMappingLayer(const String& name, Sequence* s, var params) :
	...
	automation(nullptr)
{
	recordSendMode = addEnumParameter(L"记录发送模式", L"选择录制时要执行的操作。");
	//Record Send Mode
	//Choose what to do when recording
	recordSendMode->addOption(L"不要发送", DONOTSEND)->addOption(L"发送原值", SEND_ORIGINAL)->addOption(L"发送新值", SEND_NEW);
	//Do not send
	//Send original value
	//Send new value
