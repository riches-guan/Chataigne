...
void AudioLayerClipUI::paint(Graphics& g)
{
	...
	{
		g.setFont(20);
		g.drawText(L"加载中...", getLocalBounds(), Justification::centred);//Loading
...
void AudioLayerClipUI::mouseDown(const MouseEvent& e)
{
	...
	{
		PopupMenu p;
		p.addItem(1, L"清除自动化编辑器", automationUI != nullptr);//Clear automation editor
		p.addItem(2, L"编辑信签自动化", automationUI == nullptr);//Edit enveloppe automation
		p.addItem(3, L"移除信签自动化", clip->volume->controlMode != Parameter::ControlMode::MANUAL);//Remove enveloppe automation
