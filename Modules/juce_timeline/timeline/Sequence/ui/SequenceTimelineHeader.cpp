...
void SequenceTimelineHeader::mouseUp(const MouseEvent& e)
{
	if (e.mods.isRightButtonDown() || (e.mods.isLeftButtonDown() && e.mods.isCommandDown()))
	{
		if (e.mouseWasDraggedSinceMouseDown() && selectionSpan.x >= 0)
		{
			...
			{
				PopupMenu m;
				m.addItem(1, L"选择此范围内的项目");//Select items in this range
				m.addItem(2, L"删除此范围内的项目");//Remove items in this range
				m.addSeparator();
				m.addItem(3, L"删除时间跨度");//Remove timespan
				m.addItem(4, L"插入时间跨度");//Insert timespan
