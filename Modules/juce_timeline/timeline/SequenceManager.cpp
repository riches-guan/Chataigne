...
SequenceManager::SequenceManager() :
	BaseManager(L"序列集"),//Sequences
	defaultLayerFactory(nullptr)
{
	itemDataType = "Sequence";
	helpID = "TimeMachine";

	playAllTrigger = addTrigger(L"全部播放", L"播放所有序列");//Play All  //Play all sequences
	stopAllTrigger = addTrigger(L"全部停止", L"");//Stop All  //Stop all sequences
	onlyOneSequencePlaying = addBoolParameter(L"只播放一个序列", L"如果选中，只播放一个序列，其他所有序列都会停止", false);
	//Only one sequence playing
	//If checked, as soon as one sequence is playing, all the other ones stop
}
